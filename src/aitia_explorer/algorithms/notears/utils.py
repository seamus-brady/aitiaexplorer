...
def generate_random_dag(num_nodes, num_edges, probabilistic=False, edge_coefficient_range=[0.5, 2.0]):
    ...
    if not probabilistic:
        for e_i in range(num_edges):
            ...
            adj_mat[open[choice][0]][open[choice][1]] = np.random.uniform(edge_coefficient_range[0],
                                                                          edge_coefficient_range[1]) * np.random.choice(
                [-1.0, 1.0])
        ...
        for i in range(num_nodes):
            for j in range(num_nodes):
                if ranking[i] < ranking[j]:
                    if np.random.rand() < p:
                        adj_mat[i][j] = np.random.uniform(edge_coefficient_range[0],
                                                          edge_coefficient_range[1]) * np.random.choice([-1.0, 1.0])
