...
import logging
from causal_discovery.algorithms.causal_discovery_exception import CausalDiscoveryException
...
class IC_Algorithm():
    ...
    def search(self, data, variable_types):
        self._build_graph(variable_types)
    ...
    def _build_graph(self, variable_types):
        """
        ...
        for c in self._g.nodes():
            for (a, b) in itertools.combinations(self._g.neighbors(c), 2):
                if not self._g.has_edge(a, b):
                    if c in self._g[a][c]['arrows'] and c not in self._g[b][c]['arrows'] \
                            and not (b in self._g[b][c]['arrows'] and self._g[b][c]['marked']):
                        self._g[b][c]['arrows'].append(b)
                        self._g[b][c]['marked'] = True
                        added_arrows = True
                    if c in self._g[b][c]['arrows'] and c not in self._g[a][c]['arrows'] \
                            and not (a in self._g[a][c]['arrows'] and self._g[a][c]['marked']):
    ...
    def separating_set(self, xi, xj, data=None, variable_types=None):
        if not self.separating_sets and data and variable_types:
            ...
            raise CausalDiscoveryException("Can't measure separating sets: Need data and var types.")
